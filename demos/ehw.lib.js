/**
 * Project Name:    EHW CODE_LANG Demo: LONG_DEMO_NAME
 * Project Type:    Code Demo
 * Main Project File:  controller.php
 *
 * Filename:       ehw.lib.js
 * Date Created:   02/02/22
 * Date Updated:   08/18/25
 * Programmer:     Eric L. Hepperle
 *
 * File Version:   1.00.02
 *
 * Purpose:
 *   This file serves as a JavaScript ES6 library of reusable helper functions.
 *   It is the primary library file for the project.
 *
 * Tags:
 *   JavaScript, ES6, DOM Manipulation, App, Eric L. Hepperle, Eric Hepperle
 *
 * Usage:
 *   Intended for use with controller.php.
 *
 * Sample Results:
 *   --
 *
 * Requirements:
 *   - controller.php
 *   - Browser environment
 *
 * Demonstrates:
 *   - Vanilla JavaScript
 *   - JavaScript ES6 / ECMAScript 2015 features
 *   - ES6 Date and Time object handling
 *   - String.prototype.padStart() for leading zero padding
 *
 * Future Enhancements:
 *   - Integrate fetch API or CURL to retrieve webpage content via controller.php
 * 
 * @note This file used to live in ehw-lib/ repo; migrated here 8/18/25.
 */


function getCurrentDateTime() {
  var currentdate = new Date();
  var datetime = "Last Sync: " + currentdate.getDate() + "/"
      + (currentdate.getMonth()+1)+ "/"
      + currentdate.getFullYear() + "@"
      + currentdate.getHours() + ":"
      + currentdate.getMinutes() + ":"
      + currentdate.getSeconds();
  return datetime;
}

function getDateTime(dt_fmt='US-12') {

  var out_obj = {};
  var date_obj = {};
  var time_obj = {};
  var cur_dt = new Date();

  // Date
  var yr2 = cur_dt.getFullYear().toString().substring(2);
  var mo2 = String(cur_dt.getMonth() + 1).padStart(2, 0);
  var dy2 = String(cur_dt.getDate()).padStart(2, 0);
  var date_mmddyy = `${mo2}/${dy2}/${yr2}`;
  var dow_name = Intl.DateTimeFormat('en-US', { weekday: 'long'}).format(cur_dt);
  var mo_long = cur_dt.toLocaleString('default', { month: 'long' });
  var date_full = `${dow_name}, ${mo_long} ${dy2}, ${cur_dt.getFullYear()}`;

  date_obj = {
    yr2, mo2, dy2, date_mmddyy, dow_name, mo_long, date_full
  }

  // Time
  var hrs = cur_dt.getHours(); // gets 24 hr val
  var AmOrPm = hrs >= 12 ? 'PM' : 'AM';
  
  hrs = (hrs % 12) || 12;
  var hrs_pad = hrs.toString().padStart(2, 0);

  var mins = cur_dt.getMinutes();
  var fmt_12hr = `${hrs}:${mins} ${AmOrPm}`;
  var fmt_12hr_pad = `${hrs}:${mins} ${AmOrPm}`;

  time_obj = {
    hrs, AmOrPm, mins, fmt_12hr, fmt_12hr_pad
  }

  
  out_obj.dt = cur_dt;
  out_obj['EN-12'] = {};
  out_obj['EN-12'].date = date_obj;
  out_obj['EN-12'].time = time_obj;

  return out_obj;
}
// const today = getDateTime()['EN-12'];
const today = getDateTime();

function create_results_box() {
    // create new element
    eh_div = document.createElement('div');

    // give class to element
    eh_div.id = "ehw-content";

    // define content to display
    var placeholder_cont = "This is PLACEHOLDER TEXT";

    // create a text node
    var text_node = document.createTextNode(placeholder_cont);

    // add text node to div
    eh_div.appendChild(text_node);

    // eh_div.innerHtml = "Hello!";

    // add div to body
    document.body.prepend(eh_div);
}
create_results_box();

function update_results(res) {
    
    if (res !== '' && res !== undefined) {
        res = res;
    } else {
        res = "You must pass a 'res' value to the update_results() function";
    }

    // define our content element handle
    const cont_el = document.querySelector('#ehw-content');

    // change content text
    cont_el.innerHTML = "<h4>Stringified JSON data</h4>";
    cont_el.innerHTML += res;    
}
update_results("Hello DOLLY!");
update_results("")
update_results("***")
update_results("Current Date/Time: " + today['EN-12'].date.date_full)
//alert('press enter to change the message');
//update_results('Apples are AWESOME')





// not working with foreach yet ...
function formatJSONAutos_01(obj) {
    
    let keys = Object.keys(obj);
    console.log(`keys: ${keys}`);
    
    for (let i=0; i < keys.length; i++) {
        let p = document.createElement('p');
        console.log(obj[i]);
    }
}
// formatJSONAutos_01(data);




/**
 * ------------------------------------------------------------------------
 * Disabled / Broken functions section
 * ------------------------------------------------------------------------
 * The following functions are currently disabled or non-functional.
 * They are preserved here for future debugging or re-implementation.
 * 
 * TODO:
 * - Review process_data() to fix undefined variable 'org' issue.
 * - Enhance appendData() to support dynamic container IDs and error handling.
 */

// Disabled: process_data function (requires fixing 'org' variable issue)
/*
function process_data(data){
    // create new element
    eh_div = document.createElement('div');

    // give class to element
    eh_div.id = "ehw-content";

    // define content to display
    var content = `${JSON.stringify(data)}`;
    content += `<br><br>${org}`;

    // create a text node
    var text_node = document.createTextNode("hello");

    // add text node to div
    eh_div.appendChild(text_node);

    // add div to body
    document.body.prepend(eh_div);

    // ----- Change Div Contents ---
    const cont_el = document.querySelector('#ehw-content');
    cont_el.innerHTML = "<h4>Stringified JSON data</h4>";
    cont_el.innerHTML += content;
}
// process_data(org);
*/

// Disabled: appendData function (not working with foreach yet)
/*
function appendData(data) {
  var mainContainer = document.getElementById("myData");
  for (var i = 0; i < data.length; i++) {
    var div = document.createElement("div");
    div.innerHTML = 'Name: ' + data[i].firstName + ' ' + data[i].lastName;
    mainContainer.appendChild(div);
  }
}
*/
